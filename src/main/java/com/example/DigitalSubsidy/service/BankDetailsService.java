...
public class BankDetailsService {
    ...
    public BankDetails verifyBankDetails(Long id) {
        ...
        if ("VERIFIED".equalsIgnoreCase(bankDetails.getVerificationStatus())) {
            return bankDetails;
        }

        if (bankDetails.getAccountNumber() == null || bankDetails.getAccountNumber().trim().isEmpty()) {
            throw new RuntimeException("Account number is missing or invalid");
        }
        if (bankDetails.getIfscCode() == null || bankDetails.getIfscCode().trim().isEmpty()) {
            throw new RuntimeException("IFSC code is missing or invalid");
        }
